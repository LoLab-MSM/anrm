...
def surf(mcmc, dim0, dim1, posterior_mesh, mask=True, walk=True, rejects=True, step=1, square_aspect=True, margin=0.1, bounds0=None, bounds1=None, zmin=None,
...
def objective_fn(position):
    ...
    for k in conditions.keys():
        ...
        if (k == 'BidKO'):
            ...
            if (k == 'Necr1'):
                objective.append(numpy.sum((ydata_norm[k][0][:,1] - ysim_tp) ** 2 / (2 * ydata_norm[k][0][:,2])))
            
            else:
                td_PARP = ct.calculate_time_delay(PARP_MLKL_signals[:,0], sims.tspan)
                td_MLKL = ct.calculate_time_delay(PARP_MLKL_signals[:,1], sims.tspan)
                if td_MLKL < td_PARP:
                    objective.append(numpy.sum((ydata_norm[k][0][:,1] - ysim_tp) ** 2 / (2 * ydata_norm[k][0][:,2]))+abs(td_PARP - td_MLKL))
                else:
                    objective.append(numpy.sum((ydata_norm[k][0][:,1] - ysim_tp) ** 2 / (2 * ydata_norm[k][0][:,2])))
    
    return numpy.sum(objective)
...
sims = sim.Settings()
sims.model = model
sims.tspan = numpy.linspace(0,36000,1000) #10hrs converted to seconds (1000 timepoints)
...
opts = bmc.MCMCOpts()
opts.nsteps = 100
opts.initial_values = numpy.power(10, initial_position)
...
dim0 = 0
dim1 = 1
posterior_mesh = pickle.load(open('CompII_Hypthesis_123_newtopology_2run_v40_dims_0_1_surface.pkl'))


# show prediction for C trajectory, which was not fit to
surf(mcmc, dim0, dim1, posterior_mesh, mask=True, walk=True, rejects=True, step=1, square_aspect=True, margin=0.1, bounds0=None, bounds1=None, zmin=None,
